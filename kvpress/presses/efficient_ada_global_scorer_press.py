...
@dataclass
class EfficientAdaGlobalScorerPress(BasePress):
    """
    ...
    def compress(
        ...
        n_kept = int(q_len * (1 - self.compression_ratio) * num_key_value_heads * (module.config.num_hidden_layers))
        # NOTE: current implementation only support bsz 1
        assert flatten_scores.shape[0] == 1
        
        # inter-layer Budget allocation
        gloabl_flatten_scores = torch.stack(cache.layer_scores)
        
        gloabl_flatten_scores = gloabl_flatten_scores.view(-1)
        cache_topk_idx = gloabl_flatten_scores.topk(n_kept, dim=-1).indices
        head_len = cache_metadata.head_lens[0]
        cache_topk_layer_idx = cache_topk_idx // (head_len * num_key_value_heads)

        compressed_layer_lens = torch.zeros(module.config.num_hidden_layers, dtype=torch.int32, device=keys.device)
        compressed_layer_lens.scatter_add_(
            0, cache_topk_layer_idx, torch.ones_like(cache_topk_layer_idx, dtype=torch.int32)
        )
        
        # self.layer_compress(module, hidden_states, keys, values, int(q_len * (1 - self.compression_ratio) * num_key_value_heads), 0, kwargs)
        # intra-layer Budget allocation
        for layer_idx in range(module.config.num_hidden_layers):
            self.layer_compress(module, hidden_states, keys, values, int(compressed_layer_lens[layer_idx]), layer_idx, kwargs)
